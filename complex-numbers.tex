...
\item $|z_1 + z_2|^2 = |z_1|^2 + |z_2|^2 + \overline{z_1}z_2 + z_2\overline{z_1} = |z_1|^2 + |z_2|^2 + 2\Re(z_1\overline{z_2})$.
...
$2n\pi + \theta$, where $n\in I$, where I is the set of integers. The value of argument for which $-\pi<\theta\leq \pi$ is called
...
\startformula \\arg(z) = \pi - \tan^{-1}\left(\frac{y}{|x|}\right), \arg(z)= -\pi + \tan^{-1}\left(\frac{|y|}{|x|}\right)\;\text{and}\; \arg(z) =
-\tan^{-1}\left(\frac{|y|}{x}\right)\stopformula
respecticely.
...
The complex number $\cos\theta + i\sin\theta$ is denoted by $e^{i\theta}$ or
$\arg(c)$ is $\theta$, where $c$ is the complex number.
...
\item $\arg(z_1z_2) = \arg(z_1)  + \arg(z_2) + 2n\pi$, where
 ...
 \item $|z_1 + z_2| = |z_1 - z_2| \Leftrightarrow \arg(z_1) - \arg(z_2) = \pi/2$.
...
\startplacefigure[title=Complex numbers addition]
  \startMPcode
    drawarrow (-0.5cm, 0) -- (5.5cm, 0);
    drawarrow (0, -0.5cm) -- (0, 5.5cm);
    label("$X$", (5.7cm, 0));
    label("$Y$", (0, 5.7cm));
    draw (0,0) -- (4cm,1cm);
    draw (0,0) -- (1cm,4cm);
    draw (1cm,4cm) -- (5cm,5cm);
    draw (4cm,1cm) -- (5cm,5cm);
    draw (4cm,1cm) -- (4cm,0) dashed evenly;
    draw (1cm,4cm) -- (1cm,0) dashed evenly;
    draw (5cm,5cm) -- (5cm,0) dashed evenly;
    draw (4cm,1cm) -- (5cm,1cm) dashed evenly;
    label("$O$", (-0.5cm, -0.5cm));
    label("$L$", (1cm, -0.5cm));
    label("$M$", (4cm, -0.5cm));
    label("$N$", (5cm, -0.5cm));
    label("$K$", (5.2cm, 1cm));
    label("$P_1(x_1 + iy_1)$", (3cm, 1.2cm));
    label("$P_2(x_2 + iy_2)$", (1.3cm, 4.3cm));
    label("$P(x + iy)$", (5cm, 5.2cm));
  \stopMPcode
\stopplacefigure
...
$\arg(z_1 - z_2)$ is the angle made by $\overrightarrow{QP}$ with $x$-axis which is nothing but $\theta.$
...
Let $z_1 = x_1 + iy_1, z_2 = x_2 + iy_2$ then $z = x + iy$, which divides the
previous two points in the ratio $m:n$ can be given
...
Let $A(z_1), B(z_2)$ and $C(z_3)$ be the vertices of a $\triangle ABC.$ Incenter $I(z)$ of the $\triangle ABC$ is the point of
  ...
  \item Refer \in{Figure}[fig:flbee], if $k > |z - z_2|,$ then $|z - z_1| + |z - z_2| = k$ represents an ellipse with foci at $A(z_1)$ and $B(z_2)$ and length
    ...
    \startplacefigure[title={Locus of an Ellipse}, reference=fig:flbee]
      \startMPcode
        ...
        pair P, Q; numeric r;
        P = origin; Q = 120 dir 0;
        r = 34;

        draw elliptic_path(P, Q, r);

        label.lft("$C$", P);
        label.rt("$D$", Q);
        draw P -- Q;
        drawdot (P + (0.8cm, 0)) withpen pencircle scaled 2pt;
        drawdot (Q - (0.8cm, 0)) withpen pencircle scaled 2pt;
        label.bot("$A(z_1)$", P + (0.8cm, 0));
        label.bot("$B(z_2)$", Q - (0.8cm, 0));
        label.lrt("$CD = k$", (Q - (0, 0.2cm)));
      \stopMPcode
    \stopplacefigure
  \item If $k = |z - z_2|$, then it represents the line segment joining $z_1$ and $z_2$.
  \item If $k < |z - z_2|$, then it does not represent any curve/line in Argand plane.
  \stopitemize
\item If $|z - z_1| - |z - z_2| = k$. Let $z_1$ and $z_2$ be two fixed points and $k$ be a positive real number.
  ...
  \item Refer \in{Figure}[fig:flbep], if $k\neq |z - z_2|$, then it represnts a parabola with foci at $A(z_1)$ and $B(z_2)$.
    \startplacefigure[title={Locus of a Parabola}, reference=fig:flbep]
      \startMPcode
        vardef f(expr t) = t^2 + 3 enddef;
        vardef g(expr t) =  -t^2 - 3 enddef;

        u = 1mm;
        path curve;
        curve = ((-2, f(-2)) for t =  -2 upto 2: .. (t, f(t)) endfor) scaled u;
        draw curve scaled u rotated 90;
        path c;
        c = ((-2, -f(-2)) for t =  -2 upto 2: .. (t, -f(t)) endfor) scaled u;
        draw c scaled u rotated 90;
        drawdot (-15mm, 0) withpen pencircle scaled 2pt;
        drawdot (15mm, 0) withpen pencircle scaled 2pt;
        draw (-15mm, 0) -- (15mm, 0);
        label.lft("$A(z_1)$", (-15mm, 0));
        label.rt("$B(z_2)$", (15mm, 0));
      \stopMPcode
    \stopplacefigure
  \item If $k = |z_1 - z_2|$, then it represents the straight line joining $A(z_1)$ and $B(z_2)$ but excluding the segment $AB$
    \startplacefigure[location=force]
      \startMPcode
        draw (-1cm, 0) -- (1cm, 0) dashed evenly;
        drawarrow (-1cm, 0) -- (-2cm, 0);
        drawarrow (1cm, 0) -- (2cm, 0);
...
\item $|z - z_1|^2 + |z - z_2|^2 = |z_1 - z_2|^2$. If $z_1$ and $z_2$ are two fixed points then it represents a circle with $z_1$
  ...
  \startplacefigure
    \startMPcode
      ...
      label.lft("$A(z_1)$", (-1cm, 0));
      label.rt("$B(z_2)$", (1cm, 0));
      label.bot("$O(z)$", origin);
    \stopMPcode
  \stopplacefigure
  ...
  \item If $0 < \alpha < \pi$ and $\alpha \neq \pi/2$, then it represents a segment of a circle passing through $A(z_1)$ and
    ...
    \startplacefigure
      \startMPcode
        ...
        draw (-1.414cm, -1.414cm) -- (1.414cm, -1.414cm);
        draw (-1.414cm, -1.414cm) -- (1.414cm, 1.414cm) -- (1.414cm, -1.414cm) -- cycle;
        draw angle_mark((-1.414cm, -1.414cm), (1.414cm, 1.414cm), (1.414cm, -1.414cm), 10);
        label.llft("$A(z_1)$", (-1.414cm, -1.414cm));
        label.lrt("$B(z_1)$", (1.414cm, -1.414cm));
        label.bot("$\alpha$", (1.214cm, 1.114cm));
      \stopMPcode
    \stopplacefigure
  \item If $\alpha = \pi/2$, then it represents a circle with diameter as the line segment joining $A(z_1)$ and $B(z_2)$.
    \startplacefigure
      \startMPcode
        draw fullcircle scaled 4cm;
        draw (-2cm, 0) -- (2cm, 0);
        draw (-2cm, 0) -- (1.414cm, 1.414cm);
        draw (2cm, 0) -- (1.414cm, 1.414cm);
        draw (-2cm, 0) -- (0, -2cm);
        draw (2cm, 0) -- (0, -2cm);
        label.lft("$A(z_1)$", (-2cm, 0));
        label.rt("$B(z_2)$", (2cm, 0));
        draw unitsquare scaled 5 rotated angle ((-2cm, 0) - (1.414cm, 1.414cm)) shifted (1.414cm, 1.414cm);
        draw unitsquare scaled 5 rotated angle ((2cm, 0) - (0, -2cm)) shifted (0, -2cm);
        label.bot("$\pi/2$", (1.2cm, 1.15cm));
        label.top("$\pi/2$", (0, -1.7cm));
      \stopMPcode
    \stopplacefigure
  ...
  \item If $\alpha = 0,$ then it represents the straight line joining $A(z_1)$ and $B(z_2)$.
    \startplacefigure
      \startMPcode
        ...
        label.bot("$A(z_1)$", (-1cm, 0));
        label.bot("$B(z_2)$", (1cm, 0));
        drawdot (-1cm, 0) withpen pencircle scaled 2pt;
        drawdot (1cm, 0) withpen pencircle scaled 2pt;
      \stopMPcode
    \stopplacefigure
  \stopitemize
...
\item If a point $z_1$ is a reflection of a point $z_2$ through the line $z\overline{z} + \overline{b}z = c, b\neq 0$ in the argand
  plane, then prove that $\overline{b}z_2 + b\overline{z_1} = c$.
