...
\subject[title=What Makes This Book Different?]
The license. Most books are copyrighted by publishers, while some by the authors. This is released under GNU
FDL. This means it is free as in freedom. You can modify, use it anyway you like, sell, resell, and so
on. The only condition is that if you modify then modifications have to sent back to me as well so that all
of us can benefit from that modification. You can even print, and sell it to make profit out of it.
Check license in the appendix for full details.

\useurl [git] [https://gitlab.com/shivshankar.dayal/algebra-context] [] [https://gitlab.com/shivshankar.dayal/algebra-context]
\subject[title=Make It Better]
You can help by checking for spellings, grammar, adding problems, and solutions. Perhaps writing new
chapters or extending existing ones. The book's sources are hosted at \from[git]. Send me a pull request,
and I will approve it after review. You have my thanks in advance for improving it.
...
use {\tt Emacs} with {\tt Auctex} and hope that someday I will use it in a much more
productive way someday. {\tt Emacs} macros have helped me a lot with typing this
book, and I do not see any other editor, which makes macros so easy to use.

I have used {\tt Asymptote}, \MetaPostLogo~and {\tt Tikz} for drawing all the diagrams. All of them are wonderful
...
Metapost.  \MetaPostLogo~ is equally powerful and it is closely integrated with \ConTeXt{}, which
makes it default choice for making graphics. Earlier I started with Tikz because the book
was written using Sphinx and there was no Asymptote or  \MetaPostLogo~ plugin but only Tikz.
Later I found Asymptote to be better because it can solve linear equations.
The problem with Asymptote is that because it uses \LaTeX{} so a huge installation
of Texlive is needed.  \MetaPostLogo~ on the other hand comes with standalone distribution
of \ConTeXt{}. Eventually all of the diagrams will be converted to  \MetaPostLogo~.
...
\rightaligned{Shiv Shankar Dayashru}\\
