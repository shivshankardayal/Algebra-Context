...
\startitemize[n]
\item $f(x)\pm g(x)$ is a polymial of degree $\max(p, q)$,
...
\startformula f(z) = a_nz^n + a_z^{n - 1} + \cdots + a_1z + a_0 = 0.\stopformula
...
\stoptheorem
\startproof
  We have $f(x) = a(x - \alpha_1)(x - \alpha_2)\cdots(c - \alpha_{n - 1})(x - \alpha_n)$, and we let that it
...
\startcorollary
  If roots of $x^n + a_{n - 1}x^{n - 1} + \cdots + a_1x + a_0 = 0$, where $\0\leq i\leq n- 1$ is an integer
...
where $1\leq i\leq k$. Depending on $i$ and $k$, for $i\leq k, a_i = b_ic_0 + b_{i-1}c_1 + \cdots + i_0c_i$
and for $i > k, a_i = b_ic_0 + b_{i-1}c_1 + \cdots + b_{i-k}c__k$.
